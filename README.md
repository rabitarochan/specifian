...
- **Authoring guides** — Place a `_guide.md` in any category (or at the root) to describe what to record in that category's specs and the design conventions, for both human authors and AI agents. Exposed via the Web UI, MCP tools (`get_guide`, `list_guides`), and the generated `AGENTS.md`.
...
- **Static site export** — `specifian export` writes a read-only snapshot of `.specs/` as a fully static site (no server required). All rendering (MDX, Mermaid, Excalidraw, wiki links, user components) and full-text search run in the browser. Host it anywhere — including a GitHub Pages subpath — with no configuration.
...
For an introduction to how to use these tools as an AI agent, run `specifian agents` to generate an `AGENTS.md` file (see [CLI Commands](#cli-commands) below).
